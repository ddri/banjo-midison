"""
Persistent configuration for midison.

Stored at ~/.midison/config.json (with fallback to ~/.banjo/config.json).
Currently tracks only the output directory, but the schema is extensible.
"""
...
CONFIG_DIR = Path.home() / ".midison"
LEGACY_CONFIG_DIR = Path.home() / ".banjo"
DEFAULT_OUTPUT_DIR = Path.home() / "Music" / "midison"
LEGACY_OUTPUT_DIR = Path.home() / "Music" / "banjo"
...
def get_output_directory() -> Path:
    ...
    if not DEFAULT_OUTPUT_DIR.exists() and LEGACY_OUTPUT_DIR.exists():
